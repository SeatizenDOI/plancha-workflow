...
def main ():
    for root_folder in ROOT_FOLDERS:
        ...
        for session in tqdm(os.listdir(root_folder)):
            ...
            # PDF
            for file in os.listdir(new_session_path):
                if WORD_TO_FIND in file:
                    old_file_path = Path(new_session_path, file)
                    new_file_path = Path(new_session_path, file.replace(WORD_TO_FIND, WORD_TO_WRITE))
