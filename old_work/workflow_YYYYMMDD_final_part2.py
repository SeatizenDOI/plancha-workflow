...
### Clean GPS/BATHY folder
clear_processed_session("", BATHY_PATH, "", GPS_BASE_PATH, GPS_DEVICE_PATH)

### compute GPS file (either with ppk or not)
if os.path.isdir(GPS_DEVICE_PATH) :
    if cfg_prog['gps']['use_llh_position'] == True:
        LLH_PATH, flag_gps = compute_gps(SESSION_INFO_PATH, GPS_DEVICE_PATH, GPS_BASE_PATH, SESSION_NAME, time_first_frame, FRAMES_PATH, SENSORS_PATH, rgp_station, delta_time, PPK_CONFIG_PATH, ppk_cfgs, flag_rtkfix, gpsbaseposition_mean_on_llh)
        ### plot GPS accuracy
